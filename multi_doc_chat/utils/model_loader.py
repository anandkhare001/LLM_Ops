...
class ModelLoader:
    ...
    def __init__(self):
        if os.getenv("ENV", "local").lower() != "production":
            load_dotenv()
            log.info("Runing in LOCAL mdoe: .env loaded")
        else:
    ...
    def load_embeddings(self):
        ...
        try:
            ...
            return GoogleGenerativeAIEmbeddings(model=model_name,
                                                gogle_api_key=self.api_key_mgr.get("GOOGLE_API_KEY"))
        except Exception as e:
            log.error("Error loading embedding model", error=str(e))
            raise DocumentPortalException("Failed to laod embedding model", sys)
    ...
    def load_llm(self):
       ...
       max_tokens = llm_config.get("max_ouptut_tokens", 2048)
...
if __name__ == "__main__":
    ...
    print(f"Embedding Resul: {result}")
